#!/usr/bin/env bash
# Watchdog per la run live della scena 829.
#
...
#   - pipeline morta a meta'   -> rilancia live_829.sh
#
# Uso:
#   nohup bash scripts/watchdog_829.sh > logs/watchdog_829.log 2>&1 &
#   disown
#
# Per fermarlo:
#   pkill -f watchdog_829.sh
set -uo pipefail

REPO=/home/vodka/dynamic-gsg
LOGDIR=$REPO/logs
STATE=$LOGDIR/watchdog_829_state.log
...
run_finished() {
  ...
  local main
  main=$(ls -t "$LOGDIR"/live829_*.log 2>/dev/null | head -1)
  [ -n "$main" ] && grep -q "=== FINE ===" "$main" 2>/dev/null
}

last_frame() {
  local pl
  pl=$(ls -t "$LOGDIR"/pipeline829live_*.log 2>/dev/null | head -1)
...
while true; do
  ...
  if ! pipeline_alive; then
    ...
    pkill -9 -f "live_829.sh" 2>/dev/null
    ...
    started=$(date +%s)
    cd "$REPO" || exit 1
    nohup bash "$REPO/scripts/live_829.sh" > "$LOGDIR/live829_restart_${restarts}.log" 2>&1 &
    disown
    log "  rilanciata live_829.sh"
