...
#   1. nodo Habitat        (run_sim_829.sh)       -- pubblica /camera/*
#   2. pipeline dgsg        (dgsg_live.py)          -- si iscrive, aspetta
#   3. monitor live         (watch_live_progress)   -- legge cio' che 2 scrive
#   4. recorder che cammina (run_record_829_live.sh) -- SOLO dopo che 2 e'
...
# configs/found/dgsg_live_829.py): use_dam=False e removal_opacity_threshold
...
LOGDIR=$REPO/logs
mkdir -p "$LOGDIR"
MAIN=$LOGDIR/live829_${STAMP}.log
say() { echo "[$(date +%H:%M:%S)] $*" | tee -a "$MAIN"; }

D=$REPO/experiments/FOUND/00829_live_0
mkdir -p "$D"
GT=$D/gt_events.jsonl
rm -f "$GT"
SCRIPT_PLAN="$FOUND/scripts/generated/household_experiments_scene_829.json"
cp "$SCRIPT_PLAN" "$D/household_experiments_scene_829_plan.json"
...
# (vedi Troubleshooting in README_FOUND_829.md).
pkill -f "habitat_camera_objects_node.py" 2>/dev/null
pkill -f "record_dynamic_sequence_live.py" 2>/dev/null
sleep 2

say "=== LIVE 829: nodo Habitat + pipeline + monitor + camminata ==="

# ---------------------------------------------------------- 1. simulatore
say "1/4: nodo Habitat"
"$FOUND/run_sim_829.sh" > "$LOGDIR/sim829live_${STAMP}.log" 2>&1 &
SIM=$!
say "  avviato (PID $SIM), attendo che sia pronto"
ready=0
for _ in $(seq 1 60); do
  grep -q "Habitat ROS viewer ready" "$LOGDIR/sim829live_${STAMP}.log" 2>/dev/null && { ready=1; break; }
...
DISPLAY=:1 "$PY" scripts/dynamic_gsg_real_ssim.py "$REPO/configs/found/dgsg_live_829.py" \
  > "$LOGDIR/pipeline829live_${STAMP}.log" 2>&1 &
PIPE=$!
say "  avviata (PID $PIPE), attendo il caricamento dei modelli (~45s)"
sleep 45
kill -0 "$PIPE" 2>/dev/null || { say "  ERRORE: pipeline morta subito, vedi $LOGDIR/pipeline829live_${STAMP}.log"; kill "$SIM" 2>/dev/null; exit 1; }

# ---------------------------------------------------------- 3. monitor live
say "3/4: monitor live"
"$PY" "$REPO/scripts/compute_gt_text_embeddings.py" \
  --script "$D/household_experiments_scene_829_plan.json" \
...
"$PY" "$REPO/scripts/watch_live_progress.py" \
  ...
  --scene 00829 --out-dir "$D" --interval 5 \
  > "$LOGDIR/watch829live_${STAMP}.log" 2>&1 &
WATCH=$!
say "  avviato (PID $WATCH) -> $D/00829_live.json"

# ---------------------------------------------------------- 4. camminata
say "4/4: recorder che cammina (pubblica al bridge, --gt-log attivo)"
"$FOUND/run_record_829_live.sh" --gt-log "$GT" > "$LOGDIR/record829live_${STAMP}.log" 2>&1
...
"$PY" "$REPO/scripts/report_scene_results.py" \
  ...
  --gt-log "$GT" \
  --text-embeddings "$D/gt_text_embeddings.json" \
  --scene 00829 --out-dir "$D" >> "$MAIN" 2>&1 \
...
"$PY" "$REPO/scripts/dedup_graph_objects.py" \
  --graph-stream "$D/graph_stream.jsonl" \
  --out "$D/00829_graph_deduped.json" >> "$MAIN" 2>&1 \
  && say "  dedup grafo OK" || say "  dedup grafo FALLITO"

say "=== FINE ==="
[ -f "$D/00829_summary.json" ] && "$PY" -c "
import json; d=json.load(open('$D/00829_summary.json'))
