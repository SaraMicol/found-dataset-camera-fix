...
def align_feature_color(params, color_book):
    object_idx = params['object_idx'].squeeze(1).long()
    for i, idx in tqdm(enumerate(object_idx), total=len(object_idx)):
        if idx == 0:
            params['features'][i] = params['rgb_colors'][i]
        else:
            params['features'][i] = torch.tensor(color_book[idx], dtype=torch.float32)
    return params

def visualize(scene_path, cfg, lf_cfg):
    # Load Scene Data
    ...
    # params = align_feature_color(params, color_book)
...
if __name__ == "__main__":
    ...
    visualize(scene_path, viz_cfg, lf_cfg)
